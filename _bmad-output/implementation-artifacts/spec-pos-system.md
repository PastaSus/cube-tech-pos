...
**Execution:**
- [x] Install dependencies: Tailwind CSS, React Router, axios, mysql2
- [x] Configure Tailwind CSS in the project
- [x] `server/` -- Initialize Express server with CORS, JSON parsing
- [x] `server/db/connection.js` -- MySQL connection pool using mysql2/promise, connecting to TiDB Cloud (host, port, user, password, database via env vars)
- [x] `database.sql` -- Create Products, Sales, SalesDetails tables with constraints
- [x] `server/routes/products.js` -- CRUD endpoints: GET /api/products, POST, PUT /:id, DELETE /:id
- [x] `server/routes/sales.js` -- POST /api/sales (complete transaction), GET /api/sales (list), GET /api/sales/:id (details)
- [x] `server/routes/dashboard.js` -- GET /api/dashboard (stats + recent sales)
- [x] `src/App.tsx` -- React Router setup with 4 routes: /, /products, /pos, /sales
- [x] `src/components/Layout.tsx` -- Sidebar/nav layout with responsive mobile menu
- [x] `src/pages/Dashboard.tsx` -- Display total products, inventory, sales count, recent sales table
- [x] `src/pages/Products.tsx` -- Product table with add/edit/delete modals
- [x] `src/pages/POS.tsx` -- Product grid, cart sidebar, checkout flow
- [x] `src/pages/Sales.tsx` -- Sales history table with receipt detail view
- [x] `src/api/` -- Axios/fetch wrapper for all API calls
- [x] `README.md` -- Installation guide: prerequisites, setup steps, env vars, database setup, running the app
- [x] Git setup: init repo, .gitignore (node_modules, .env), initial commit
