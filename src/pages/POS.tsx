...
export default function POS() {
  ...
  const loadProducts = useCallback(() => {
    productsApi
      ...
      .catch(() =>
        setCheckoutMessage({ type: "error", text: "Failed to load products" }),
      );
  }, []);
  ...
  const handleCheckout = async () => {
    ...
    try {
      ...
      setCheckoutMessage({
        type: "success",
        text: `Sale complete! Receipt: ${sale.receipt_number}`,
      });
      setCart([]);
      loadProducts();
    } catch (err) {
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-[1fr_20rem] gap-6 lg:h-full">
